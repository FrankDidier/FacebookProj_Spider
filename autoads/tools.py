...
def get_network_info():
    ...
    network.append(
        {
            "MAC": ":".join([mac[e:e + 2] for e in range(0, 11, 2)])  # 无线局域网适配器 WLAN 物理地址
        }
    )
...
def getCombinNumber():
    a = get_network_info()
    if a and len(a) > 0 and a[0]['MAC']:
        mac = a[0]['MAC']
    else:
        mac = 'B8:86:87:3E:AA:E0'

    b = get_CPU_info()
    if b and len(b) > 0 and b[0]['Serial Number']:
        serial_number = b[0]['Serial Number']
    else:
        serial_number = 'BFEBFBFF000306C3'

    c = get_disk_info()
    if c and len(c) > 0 and c[0]['Serial']:
        disk_serial = c[0]['Serial']
    else:
        disk_serial = 'S21VNXAGB02318T'

    d = get_mainboard_info()
    if d and len(d) > 0:
        mainboard = d[0]
    else:
        mainboard = 'NBMV2110015437053D3400'

    machinecode_str = ""
    machinecode_str = machinecode_str + mac + serial_number + disk_serial + mainboard
    log.info(
        f'machinecode_str={machinecode_str} | mac={mac} | Serial Number={serial_number} | Serial={disk_serial} | mainboard={mainboard}')
    selectindex = [3, 6, 9, 15, 20, 22, 27, 29, 31, 35, 39, 44, 46, 48, 50, 53, 55, 58]  # 18位机器码
    macode = ""
    for i in selectindex:  # 根据字符串位数筛选部分字符,如果不是数字或者字母，就使用下一个字符
        try:
            while True:
                if len(machinecode_str) > i:
                    if re.match('[0-9A-Za-z]', machinecode_str[i]):
...
def get_greet_threading_count(config_from_newest=None):
    if config_from_newest:
        _config = config_from_newest
    else:
        _config = config

    ads = get_ads_id()
    if _config.account_nums > len(ads):
        return len(ads)
    else:
        return _config.account_nums
...
def scroll_element_into_middle(driver,element):
...
def filter_by(data, **kwargs):
    params = kwargs
    for key in params:
        if data.get(key) != params[key]:
            return False
    return True


def dictToObj(dictObj, item):
    if not isinstance(dictObj, dict):
        return dictObj
    item = copy.copy(item)
    for k, v in dictObj.items():
        item[k] = dictToObj(v, item)
    return item


def load_items(item, begin=0):
    """
    加载数据，按照一行一行的规则给到浏览器去发请求
    :return:
    """
    table = abspath(item.table_name)  # 这里只是一个目录，我们需要把目录中的文件都要过滤一遍来获取到请求
...
def get_page_data_mutilxpath(browser, list_xpath,is_log=False):
    elements = []
    if list_xpath and browser and len(list_xpath):
        for xpath in list_xpath:
            elements = browser.find_elements('xpath',
                                             xpath)
    ...
    if not len(elements):
        if is_log:
            log.error(f'当前多个xpath规则都找不到数据，请检查规则 | {list_xpath}')
        return []
...
def _is_listlike(x):
    ...
    True
    """
    return hasattr(x, "__iter__") and not isinstance(x, (bytes,))
